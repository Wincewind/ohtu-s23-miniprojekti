...
def delete_books_by_id(book_id: list[int]):
    ...
    try:
        if len(book_id) == 1:
            db.session.execute(
                text("DELETE FROM Books WHERE id = :book_id"), {"book_id": book_id[0]}
            )
            db.session.commit()

        else:
            book_id_strings = ", ".join([f":book_id_{i}" for i in range(len(book_id))])
            sql_query = f"DELETE FROM Books WHERE id IN ({book_id_strings})"
            id_string_parameters = {f"book_id_{i}": id for i, id in enumerate(book_id)}
